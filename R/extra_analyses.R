...
## Sex strata any GBD ---------------------------------------------------------
sex_strata <- function(data) {
    # Define covariates and models
    covars2 <- c("cereal_refined_weekly", "whole_grain_weekly", "mixed_dish_weekly",
                 "dairy_weekly", "fats_weekly", "fruit_weekly", "nut_weekly",
                 "veggie_weekly", "potato_weekly", "egg_weekly",
                 "non_alc_beverage_weekly", "alc_beverage_weekly", "snack_weekly",
    ...
    # Function to run analysis for a given sex
    run_analysis <- function(sex_group) {
        subset_data <- data %>%
            subset(sex == sex_group)
    ...
    all_results <- map(0:1, run_analysis) |>
        bind_rows()
